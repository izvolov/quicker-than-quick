...
![bg cover opacity:5%](непрактична2_crop.png)
...
![bg fit](no-opt-int32-10-100-shuffle.svg)
...
![bg fit](no-opt-int8-5-30-shuffle.svg)
...
![bg fit](no-opt-int32-10-1000000-shuffle-norm.svg)
...
![bg fit](no-opt-int8-10-10000000-shuffle-norm.svg)
...
![bg](прокрастинация.jpg)

---

<!--
Тут же я отправил на stdcpp.ru предложение добавить в алгоритмы поразрядную сортировку.
...
![bg opacity:10%](идея.jpg)

# Реализация `std::stable_sort` в libc++ (LLVM)

![w:2000](stable-sort.svg)
...
![bg fit](no-opt-int32-10-1000000-ascending-norm.svg)
...
![bg fit](no-opt-int32-10-1000000-single-norm.svg)
...
![bg fit](radix-opt-int32-10-1000-shuffle-compare.svg)
...
![bg fit](no-opt-int32-10-1000000-ascending-compare.svg)
...
![bg fit](opt-int32-10-1000000-ascending-compare.svg)
...
![bg fit](opt-int128-10-1000000-outlier-compare.svg)
...
![bg fit](limited-int128-10-1000000-outlier-compare.svg)
...
![bg fit](limited-int8-10-1000000-shuffle-compare.svg)
...
![bg fit](limited-int16-10-1000000-shuffle-compare.svg)
...
![bg fit](limited-int32-10-1000000-shuffle-compare.svg)
...
![bg fit](limited-int64-10-1000000-shuffle-compare.svg)
...
Если используете libc++ и сортируете целые — `std::sort` не всегда лучший выбор. Раньше никто в здравом уме не использовал `std::stable_sort` для целых чисел. Теперь же есть осязаемая альтернатива.
...
Выигрыш от 2 до 10 раз, и лучше всего, если размер сортируемого диапазона попадает между порогами, которые мы подобрали выше.
-->
